...
const normalizeLabel = (key = "") => key.replace(/^\//, "").replace(/_/g, " ").trim() || "Unknown";
...
const DocumentManager = () => {
  ...
  if (selectedDoc?.metadata?.size_bytes) {
    metadataEntries.push(["File size", formatBytes(selectedDoc.metadata.size_bytes)]);
  }
  Object.entries(pdfMetadata).forEach(([key, val]) => {
    metadataEntries.push([normalizeLabel(key), `${val ?? "-"}`]);
  });
  const rows = [];
  for (let i = 0; i < metadataEntries.length; i += 2) {
    rows.push(metadataEntries.slice(i, i + 2));
  }
        ...
        <div className="col-span-2 bg-gray-900 border border-gray-800 rounded-lg p-4 overflow-auto space-y-4">
          ...
          {selectedDoc && (
            <>
              ...
              <div>
                <h4 className="text-sm font-semibold mb-1 text-indigo-300">Metadata tài liệu</h4>
                {rows.length ? (
                  <div className="space-y-2">
                    {rows.map((pair, idx) => (
                      <div key={pair.map((item) => item[0]).join("-") || idx} className="grid grid-cols-2 gap-3">
                        {pair.map(([key, val]) => (
                          <div
                            key={`${key}-${val}`}
                            className="bg-gray-950 border border-gray-800 rounded p-3 flex flex-col gap-1 text-xs"
                          >
                            <div className="text-gray-400 uppercase tracking-wide text-[10px]">{key}</div>
                            <div className="text-sm text-white break-words">{val || "-"}</div>
                          </div>
                        ))}
                        {pair.length === 1 && (
                          <div className="rounded border border-dashed border-gray-800 opacity-0" aria-hidden="true" />
                        )}
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="text-sm text-gray-500">Không có metadata.</div>
                )}
              </div>
